...
func input() bool {
	...
	cmd := exec.Command("rm", "-rf", gopath+"/src/github.com/8treenet/gotree/gotree/examples")
	cmd.Output()
	if err := os.Mkdir(gopath+"/src/github.com/8treenet/gotree/gotree/examples", os.ModePerm); err != nil {
		fmt.Println(err)
		return true
	}
	cmd = exec.Command("cp", gopath+"/src/examples/learning.sql", gopath+"/src/github.com/8treenet/gotree/gotree/examples/learning.sql")
	cmd.Output()
	cmd = exec.Command("cp", "-r", gopath+"/src/examples/dao", gopath+"/src/github.com/8treenet/gotree/gotree/examples/dao")
	cmd.Output()
	cmd = exec.Command("cp", "-r", gopath+"/src/examples/app", gopath+"/src/github.com/8treenet/gotree/gotree/examples/app")
	cmd.Output()
	cmd = exec.Command("cp", "-r", gopath+"/src/examples/protocol", gopath+"/src/github.com/8treenet/gotree/gotree/examples/protocol")
	cmd.Output()

	if err := iputGoRecursion(gopath + "/src/github.com/8treenet/gotree/gotree/examples/dao"); err != nil {
		fmt.Println(err)
		return true
	}
	if err := iputGoRecursion(gopath + "/src/github.com/8treenet/gotree/gotree/examples/app"); err != nil {
		fmt.Println(err)
		return true
	}
	if err := iputGoRecursion(gopath + "/src/github.com/8treenet/gotree/gotree/examples/protocol"); err != nil {
		fmt.Println(err)
		return true
	}
	return true
...
func newProject() {
	...
	gopath := os.Getenv("GOPATH")
	if fileExists(gopath + "/src/" + project) {
		fmt.Println(fmt.Sprintf("\x1b[0;%dm%s\x1b[0m", 31, os.Args[len(os.Args)-1]+" already exists."))
		return
	}
	cmd := exec.Command("cp", "-r", gopath+"/src/github.com/8treenet/gotree/gotree/examples", gopath+"/src/"+project)
	_, err := cmd.Output()
	if err != nil {
